...
USE_SPLITTING = True     # <- tu mets False quand tu veux désactiver
SPLIT_N = 10000             # <- nombre de copies (2,5,10,...)
# --- Splitting dans le plomb après 1ère interaction ---
USE_LEAD_SPLIT = True
LEAD_SPLIT_N = 10000   # ex: 20, 50, 100 (évite 1000 sinon explosion)
    ...
    while neutron.alive:
        ...
        if l_collision < dist_boundary:
            ...
            if random.random() < Pabs:
                ...
                # on recommence la boucle avec la nouvelle direction
                continue
...
def try_score_detector_M(neutron, energies_M=None):
    ...
    if not (detector_M_y[0] < yM < detector_M_y[1]):
        return False
    if neutron.energy>10.0:
        ok = (random.random() < 0.001*100)
    else:
...
def enter_iron(neutron, hit_per_detector, absorptions_iron, energies_G=None):
    ...
    if res == "left":
        # sortie vers le vide : peut scorer G + peut toucher l'or (côté droit de l'or)
        ...
        if y_gold is not None and inside_gold(y_gold):
            ...
            nudge(neutron)  # le mettre clairement dans l'or
            return "continue"

        return "dead"

    if res == "right":
        # sort vers PE (frontière x_J)
        nudge(neutron)
        return "continue"
...
def enter_pe(neutron, absorptions_pe):
    ...
    # left -> retourne fer ; right -> va plomb
    if res == "left":
        stats["exit_pe_left"] += neutron.w
    elif res == "right":
        stats["exit_pe_right"] += neutron.w
    nudge(neutron)
    return "continue"

def enter_lead(neutron, absorptions_lead):
    stats["enter_pb"] += neutron.w
    ...
    # left -> retourne PE ; right -> sort côté M
    if res == "left":
        stats["exit_pb_left"] += neutron.w
    elif res == "right":
        stats["exit_pb_right"] += neutron.w
...
def enter_gold(neutron, hit_per_detector, absorptions_gold, energies_G=None):
    # IMPORTANT : pour l'or, ton transport_region gère top/bottom aussi
    ...
    if res == "left":
        # sort côté gauche de l'or -> tu t'en fous (retour vers l'amont), on tue
        return "dead"

    if res in ("top", "bottom"):
        # sort en y, il peut aller frapper le fer (plan x_H) mais pas G
    ...
    if res == "right":
        # sort côté droit de l'or : peut scorer G puis aller vers fer
        try_score_detector_G(neutron, hit_per_detector, energies_G)
...
def run_one_neutron(neutron,
                    ...
                    energies_G=None):
    """
    Fait toute la vie d'un neutron après les fentes.
    Retourne True si hit_M, False sinon.
    """

    # injection (or ou fer)
    if not initial_injection(neutron, hit_per_detector, energies_G):
        return False

    while neutron.alive:
        x, y = neutron.current_x, neutron.current_y

        # Si le neutron est dans le fer
        if (x_H - 1e-9) <= x <= (x_J + 1e-9) and get_region(x, y) == REGION_IRON:
            status = enter_iron(neutron, hit_per_detector, absorptions_iron, energies_G)
            if status == "dead":
                return False
            continue

        # PE
        if get_region(x, y) == REGION_PE:
        ...
        # Or
        if get_region(x, y) == REGION_GOLD:
            status = enter_gold(neutron, hit_per_detector, absorptions_gold, energies_G)
            if status == "dead":
                return False
            continue

        # Vide après plomb : test M puis mort
        if x >= x_L - 1e-9 and get_region(x, y) == REGION_VACUUM:
            return propagate_to_M_from_xL(neutron, energies_M)

        # Sinon dehors / cas non géré
        neutron.alive = False
        return False
